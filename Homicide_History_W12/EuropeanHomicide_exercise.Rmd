...
```{r homicideplots by country, eval = TRUE}
homicide_stats +
  geom_line() + 
  facet_wrap(~Entity)+
...
```{r duration of reign among Danish kings, eval = TRUE}
...
monarchs_midyear_vis +
  geom_smooth()+
  labs(title = "Duration of reign among Danish kings",
       x = "Year",
       y = "Duration of reign")
```
...
Before answering the question the word civilized must first be defined. Let's say that the less people kill each other in illegal ways, equals a more civilized society or people. If that is true the visualizations show that we are more civilized today, because there is less homicide. I think, however, that such a conclusion, on the basis of the given data-sets and the simplistic definition of "civilized", might be a little to ambitious. More context is needed. What if the drop in recorded homicides was because less and less killers where caught? If that was the case we might deem Western Europe less and less civilized. That is probably not the case, but historical context is important, and with the right context the very significant drop in homicides in Western Europe could support the idea of a increasingly civilized Western Europe. 